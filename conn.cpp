...
void conn::init_clt(int sock_fd, const sockaddr_in &client_addr) {
    m_clt_fd = sock_fd;
    m_clt_address = client_addr;
}

// 参数：负载均衡服务器创建的与服务端的 socket 连接、服务端地址
void conn::init_srv(int sock_fd, const sockaddr_in &server_addr) {
    m_srv_fd = sock_fd;
...
RET_CODE conn::read_clt() {
    ...
    while (true) {
        ...
        bytes_read = recv(m_clt_fd, m_clt_buf + m_clt_read_idx, BUF_SIZE - m_clt_read_idx, 0);
        if (bytes_read == -1) {
...
RET_CODE conn::read_srv() {
    ...
    while (true) {
        ...
        bytes_read = recv(m_srv_fd, m_srv_buf + m_srv_read_idx, BUF_SIZE - m_srv_read_idx, 0);
        if (bytes_read == -1) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                break;
            }
...
RET_CODE conn::write_srv() {
    ...
    while (true) {
        ...
        bytes_write = send(m_srv_fd, m_clt_buf + m_clt_write_idx, m_clt_read_idx - m_clt_write_idx, 0);
        if (bytes_write == -1) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                return TRY_AGAIN;
            }
...
RET_CODE conn::write_clt() {
    ...
    while (true) {
        ...
        bytes_write = send(m_clt_fd, m_srv_buf + m_srv_write_idx, m_srv_read_idx - m_srv_write_idx, 0);
        if (bytes_write == -1) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                return TRY_AGAIN;
            }
